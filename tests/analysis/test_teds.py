...
def test_html_comment_inside_table_does_not_crash():
    """Real model output often contains HTML comments; lxml exposes their
    ``.tag`` as a callable, which previously crashed ``_convert``."""
    with_comment = """
    <table>
      <!-- generated by model -->
      <tbody>
        <tr><td>1</td><td>2</td><!-- trailing --></tr>
      </tbody>
    </table>
    """
    plain = "<table><tbody><tr><td>1</td><td>2</td></tr></tbody></table>"
    # Comments are ignored, so the structure matches exactly.
    assert teds_score(with_comment, plain) == pytest.approx(1.0)
